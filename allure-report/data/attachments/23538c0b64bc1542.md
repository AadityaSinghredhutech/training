...
- Name: secludedCartProg.spec.js >> Client App login
- Location: tests\secludedCartProg.spec.js:7:1
...
Call log:
  - waiting for locator('.ta-results').locator('button').nth(10)
    ...
    - generic [ref=e41]:
      ...
      - generic [ref=e48]:
        ...
        - generic [ref=e76]:
          ...
          - generic [ref=e79]:
            - generic [ref=e80]: hanuchizuru@gmail.com
            - textbox [ref=e81]: hanuchizuru@gmail.com
            - generic [ref=e83]:
...
```ts
  ...
  2  |     constructor(page){
  3  |         this.page=page;
  4  |         this.countrySection = page.locator("[placeholder*='Country']");
  5  |         this.dropdown = page.locator(".ta-results");
  6  |     
  7  |     }
  8  | 
  9  |     async dropdownSelect(countryName){
  10 |         await this.countrySection.pressSequentially(countryName,{delay:150});
  11 |         await this.dropdown.waitFor();
  12 |         const optionCount=await this.dropdown.locator("button").count();
  13 |         for(let i=0;i<optionCount;i++){
> 14 |          const text=await this.dropdown.locator("button").nth(i).textContent();
     |                                                                  ^ Error: locator.textContent: Test timeout of 40000ms exceeded.
  15 |           if(text.trim()===countryName){
  16 |           await this.dropdown.locator("button").nth(i).click();
  17 |            break;
  18 | }
  19 | }
  20 |     }
